...
local function set_wielder_formspec(data, meta)
	meta:set_string("formspec",
			"invsize[9,"..(6+data.wield_inv_height)..";]"..
...
local can_tool_dig_node = function(nodename, toolcaps, toolname)
	--mcl_pipeworks.logger("can_tool_dig_node() STUB nodename="..tostring(nodename).." toolname="..tostring(toolname).." toolcaps: "..dump(toolcaps))
...
local function wielder_on(data, wielder_pos, wielder_node)
	...
	local virtplayer = mcl_pipeworks.create_fake_player({
		name = data.masquerade_as_owner and wielder_meta:get_string("owner")
			or ":mcl_pipeworks:" .. minetest.pos_to_string(wielder_pos),
	...
	if data.eject_drops then
		for i, stack in ipairs(inv:get_list("main")) do
			if not stack:is_empty() then
				mcl_pipeworks.tube_inject_item(wielder_pos, wielder_pos, dir, stack)
...
if mcl_pipeworks.enable_node_breaker then
	local data
	-- see after end of data table for other use of these variables
	local name_base = "mcl_pipeworks:nodebreaker"
	local wield_inv_name = "pick"
	data = {
		name_base = name_base,
		description = "Node Breaker",
		texture_base = "mcl_pipeworks_nodebreaker",
		...
		act = function(virtplayer, pointed_thing)
			...
			if on_use then
				--mcl_pipeworks.logger(dname.."invoking on_use "..tostring(on_use))
				...
				if can_tool_dig_node(under_node.name,
					...
					--mcl_pipeworks.logger(dname.."couldn't dig node!")
				end
			end
	...
	minetest.register_craft({
		...
		recipe = {
			{ "mcl_pipeworks:gear", "mcl_pipeworks:gear",   "mcl_pipeworks:gear"    },
			{ "mcl_core:stone", "mesecons_pistons:piston_normal_off",   "mcl_core:stone" },
			{ "group:wood",    "mesecons:redstone",  "group:wood" },
		}
	})
	-- aliases for when someone had technic installed, but then uninstalled it but not mcl_pipeworks
	minetest.register_alias("technic:nodebreaker_off", "mcl_pipeworks:nodebreaker_off")
	minetest.register_alias("technic:nodebreaker_on", "mcl_pipeworks:nodebreaker_on")
	minetest.register_alias("technic:node_breaker_off", "mcl_pipeworks:nodebreaker_off")
	minetest.register_alias("technic:node_breaker_on", "mcl_pipeworks:nodebreaker_on")
	-- turn legacy auto-tree-taps into node breakers
	dofile(mcl_pipeworks.modpath.."/legacy.lua")

	-- register LBM for transition to cheaper node breakers
	local lbm_id = "mcl_pipeworks:refund_node_breaker_pick"
	minetest.register_lbm({
		...
		action = function(pos, node)
			mcl_pipeworks.logger(lbm_id.." entry, nodename="..node.name)
...
if mcl_pipeworks.enable_deployer then
	register_wielder({
		name_base = "mcl_pipeworks:deployer",
		description = "Deployer",
		texture_base = "mcl_pipeworks_deployer",
	...
	-- aliases for when someone had technic installed, but then uninstalled it but not mcl_pipeworks
	minetest.register_alias("technic:deployer_off", "mcl_pipeworks:deployer_off")
	minetest.register_alias("technic:deployer_on", "mcl_pipeworks:deployer_on")
end

if mcl_pipeworks.enable_dispenser then
	register_wielder({
		name_base = "mcl_pipeworks:dispenser",
		description = "Dispenser",
		texture_base = "mcl_pipeworks_dispenser",
		texture_stateful = { front = true },
		tube_connect_sides = { back=1 },
		tube_permit_anteroposterior_insert = true,
		stack_max = 64,
