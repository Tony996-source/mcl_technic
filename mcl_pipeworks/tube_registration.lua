...
local S = minetest.get_translator("mcl_pipeworks")

local tubenodes = {}
mcl_pipeworks.tubenodes = tubenodes

minetest.register_alias("mcl_pipeworks:tube", "mcl_pipeworks:tube_000000")
...
local default_noctrs = { "mcl_pipeworks_tube_noctr.png" }
local default_plain = { "mcl_pipeworks_tube_plain.png" }
local default_ends = { "mcl_pipeworks_tube_end.png" }
...
local register_one_tube = function(name, tname, dropname, desc, plain, noctrs, ends, short, inv, special, connects, style)
	...
	short = short or "mcl_pipeworks_tube_short.png"
	inv = inv or "mcl_pipeworks_tube_inv.png"
	...
	for _, v in ipairs(connects) do
		mcl_pipeworks.table_extend(outboxes, mcl_pipeworks.tube_boxes[v])
		table.insert(outsel, mcl_pipeworks.tube_selectboxes[v])
	...
	local tgroups = {pickaxey=1,axey=1, handy=1, swordy=1, tube = 1, tubedevice = 1, not_in_creative_inventory = 1}
	...
	if #connects == 0 then
		tgroups = {pickaxey=1,axey=1, handy=1, swordy=1, tube = 1, tubedevice = 1}
		tubedesc = desc
		iimg=inv
		outimgs = {
			short, short,
	...
	local nodedef = {
		...
		on_rotate = false,
		check_for_pole = mcl_pipeworks.check_for_vert_tube,
		check_for_horiz_pole = mcl_pipeworks.check_for_horiz_tube,
	...
	for key, value in pairs(special) do
		...
		elseif key == "tube" then
			...
			nodedef[key] = mcl_pipeworks.table_recursive_replace(value, "#id", tname)
		end
	end
...
mcl_pipeworks.register_tube = function(name, def, ...)
	if type(def) == "table" then
		...
		assert(type(def) == "string", "invalid arguments to mcl_pipeworks.register_tube")
