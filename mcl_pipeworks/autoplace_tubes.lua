...
local function nodeside(node, tubedir)
	...
	local topdir = mcl_pipeworks.facedir_to_top_dir(node.param2)
	local top = mcl_pipeworks.vector_dot(topdir, tubedir)
	if top == 1 then
		return "top"
	elseif top == -1 then
		return "bottom"
	...
	local rightdir = mcl_pipeworks.facedir_to_right_dir(node.param2)
	local right = mcl_pipeworks.vector_dot(rightdir, tubedir)
	if right == 1 then
		return "right"
	else
		return "left"
...
function mcl_pipeworks.after_place(pos)
...
function mcl_pipeworks.on_rotate(pos, node, user, mode, new_param2)
...
if minetest.get_modpath("mesecons_mvps") then
	mesecon.register_on_mvps_move(function(moved_nodes)
		for _, n in ipairs(moved_nodes) do
			mcl_pipeworks.scan_for_tube_objects(n.pos)
			mcl_pipeworks.scan_for_tube_objects(n.oldpos)
		end
	end)
end
