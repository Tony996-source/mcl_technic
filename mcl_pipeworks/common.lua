...
function mcl_pipeworks.vector_cross(a, b)
	return {
		x = a.y * b.z - a.z * b.y,
		y = a.z * b.x - a.x * b.z,
		z = a.x * b.y - a.y * b.x
...
function mcl_pipeworks.vector_dot(a, b)
...
function mcl_pipeworks.facedir_to_top_dir(facedir)
...
function mcl_pipeworks.facedir_to_right_dir(facedir)
	return mcl_pipeworks.vector_cross(
		mcl_pipeworks.facedir_to_top_dir(facedir),
		minetest.facedir_to_dir(facedir)
	)
end

local directions = {}
mcl_pipeworks.directions = directions
...
function directions.dir_to_side(dir)
	local c = mcl_pipeworks.vector_dot(dir, vector.new(1, 2, 3)) + 4
...
--[[function mcl_pipeworks.string_split(str, sep)
	local fields = {}
	local index = 1
	local expr = "([^"..sep.."])+"
...
function mcl_pipeworks.string_startswith(str, substr)
...
function mcl_pipeworks.table_contains(tbl, element)
...
function mcl_pipeworks.table_extend(tbl, tbl2)
	local index = #tbl + 1
...
function mcl_pipeworks.table_recursive_replace(tbl, pattern, replace_with)
	if type(tbl) == "table" then
		local tbl2 = {}
		for key, value in pairs(tbl) do
			tbl2[key] = mcl_pipeworks.table_recursive_replace(value, pattern, replace_with)
		end
		return tbl2
...
local fs_helpers = {}
mcl_pipeworks.fs_helpers = fs_helpers
function fs_helpers.on_receive_fields(pos, fields)
	local meta = minetest.get_meta(pos)
	for field, value in pairs(fields) do
		if mcl_pipeworks.string_startswith(field, "fs_helpers_cycling:") then
			local l = field:split(":")
...
function mcl_pipeworks.load_position(pos)
...
function mcl_pipeworks.create_fake_player(def, is_dynamic)
