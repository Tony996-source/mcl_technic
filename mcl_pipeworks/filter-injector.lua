...
local function set_filter_formspec(data, meta)
	...
	if data.digiline then
		formspec = "size[8,2.7]"..
			"item_image[0,0;1,1;mcl_pipeworks:"..data.name.."]"..
...
local function grabAndFire(data,slotseq_mode,exmatch_mode,filtmeta,frominv,frominvname,frompos,fromnode,filterfor,fromtube,fromdef,dir,fakePlayer,all,digiline)
	...
	for _, spos in ipairs(sposes) do
			...
			if doRemove > 0 then
				...
				local item1 = mcl_pipeworks.tube_inject_item(pos, start_pos, dir, item, fakePlayer:get_player_name())
...
local function punch_filter(data, filtpos, filtnode, msg)
	...
	local fakePlayer = mcl_pipeworks.create_fake_player({
		name = owner
	})
	local dir = mcl_pipeworks.facedir_to_right_dir(filtnode.param2)
	...
	local input_special_cases = {
		["mcl_technic:mv_furnace"] = "dst",
		["mcl_technic:mv_furnace_active"] = "dst",
		["mcl_technic:mv_electric_furnace"] = "dst",
		["mcl_technic:mv_electric_furnace_active"] = "dst",
		["mcl_technic:mv_alloy_furnace"] = "dst",
		["mcl_technic:mv_alloy_furnace_active"] = "dst",
		["mcl_technic:mv_compressor"] = "dst",
		["mcl_technic:mv_compressor_active"] = "dst",
		["mcl_technic:mv_extractor"] = "dst",
		["mcl_technic:mv_extractor_active"] = "dst",
		["mcl_technic:mv_grinder"] = "dst",
		["mcl_technic:mv_grinder_active"] = "dst",
		["mcl_technic:tool_workshop_mv"] = "src",
		["mcl_technic:mv_freezer"] = "dst",
		["mcl_technic:mv_freezer_active"] = "dst",
		["mcl_technic:mv_battery_box"] = "dst",
		
		["mcl_technic:hv_furnace"] = "dst",
		["mcl_technic:hv_furnace_active"] = "dst",
		["mcl_technic:hv_electric_furnace"] = "dst",
		["mcl_technic:hv_electric_furnace_active"] = "dst",
		["mcl_technic:hv_alloy_furnace"] = "dst",
		["mcl_technic:hv_alloy_furnace_active"] = "dst",
		["mcl_technic:hv_compressor"] = "dst",
		["mcl_technic:hv_compressor_active"] = "dst",
		["mcl_technic:hv_extractor"] = "dst",
		["mcl_technic:hv_extractor_active"] = "dst",
		["mcl_technic:hv_grinder"] = "dst",
		["mcl_technic:hv_grinder_active"] = "dst",
		["mcl_technic:tool_workshop_hv"] = "src",
		["mcl_technic:hv_freezer"] = "dst",
		["mcl_technic:hv_freezer_active"] = "dst",
		["mcl_technic:hv_battery_box"] = "dst"
	}

	-- make sure there's something appropriate to inject the item into
	local todir = mcl_pipeworks.facedir_to_right_dir(filtnode.param2)
...
for _, data in ipairs({
	...
	local node = {
		...
		tiles = {
			"mcl_pipeworks_"..data.name.."_top.png",
			"mcl_pipeworks_"..data.name.."_top.png",
			"mcl_pipeworks_"..data.name.."_output.png",
			"mcl_pipeworks_"..data.name.."_input.png",
			"mcl_pipeworks_"..data.name.."_side.png",
			"mcl_pipeworks_"..data.name.."_top.png",
		},
		paramtype2 = "facedir",
		...
		allow_metadata_inventory_put = function(pos, listname, index, stack, player)
			if not mcl_pipeworks.may_configure(pos, player) then
				return 0
			end
	...
	minetest.register_node("mcl_pipeworks:"..data.name, node)
end

minetest.register_craft( {
	output = "mcl_pipeworks:filter 2",
	recipe = {
	{ "mcl_core:iron_ingot", "mcl_core:iron_ingot", "mcl_pipeworks:plastic_sheet" },
...
minetest.register_craft( {
	output = "mcl_pipeworks:mese_filter 2",
	recipe = {
	        { "mcl_core:iron_ingot", "mcl_core:iron_ingot", "mcl_pipeworks:plastic_sheet" },
	        { "group:stick", "mesecons_torch:redstoneblock", "mcl_pipeworks:plastic_sheet" },
	        { "mcl_core:iron_ingot", "mcl_core:iron_ingot", "mcl_pipeworks:plastic_sheet" }
	},
})
...
minetest.register_lbm({
	...
	name = "mcl_pipeworks:give_back_old_filter_items",
	nodenames = {"mcl_pipeworks:filter", "mcl_pipeworks:mese_filter"},
