local constant_digit_count = mcl_technic.config:get("constant_digit_count")
...
function mcl_technic.pretty_num(num)
...
-- some unittests
assert(mcl_technic.pretty_num(-0) == "0 ")
assert(mcl_technic.pretty_num(0) == "0 ")
assert(mcl_technic.pretty_num(1234) == "1234 ")
assert(mcl_technic.pretty_num(123456789) == "123.5 M")


-- used to display power values
function mcl_technic.EU_string(num)
	return mcl_technic.pretty_num(num) .. "EU"
end


--- Same as minetest.swap_node, but only changes name
-- and doesn't re-set if already set.
function mcl_technic.swap_node(pos, name)
...
function mcl_technic.refill_RE_charge(stack)
	local max_charge = mcl_technic.power_tools[stack:get_name()]
	if not max_charge then return stack end
	mcl_technic.set_RE_wear(stack, max_charge, max_charge)
...
function mcl_technic.get_or_load_node(pos)
...
mcl_technic.tube_inject_item = mcl_pipeworks.tube_inject_item or function(pos, start_pos, velocity, item)
	local tubed = mcl_pipeworks.tube_item(vector.new(pos), item)
...
function mcl_technic.trace_node_ray(pos, dir, range)
...
function mcl_technic.trace_node_ray_fat(pos, dir, range)
	local x_step = dir.x > 0 and 1 or -1
	local y_step = dir.y > 0 and 1 or -1
	local z_step = dir.z > 0 and 1 or -1
