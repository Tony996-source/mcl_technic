...
local function set_quarry_formspec(meta)
	...
	local formspec = "size[6,4.3]"..
		...
		mcl_formspec.get_itemslot_bg(0,1,4,3)..
		"item_image[4.8,0;1,1;mcl_technic:hv_quarry]"..
...
local function quarry_handle_purge(pos)
	...
	for _,stack in ipairs(inv:get_list("cache")) do
		...
		if stack then
			...
			if item then
				mcl_technic.tube_inject_item(pos, pos, quarry_eject_dir, item)
				stack:clear()
...
local function quarry_run(pos, node)
	...
	if meta:get_int("enabled") and meta:get_int("HV_EU_input") >= quarry_demand and meta:get_int("purge_on") == 0 then
		...
		while nd ~= diameter*diameter * (quarry_dig_above_nodes+1+quarry_max_depth) do
			...
			local dignode
			if can_dig then
				dignode = mcl_technic.get_or_load_node(digpos) or minetest.get_node(digpos)
				local dignodedef = minetest.registered_nodes[dignode.name] or {diggable=false}
				-- doors mod among other thing does NOT like a nil digger...
				local fakedigger = mcl_pipeworks.create_fake_player({
					name = owner
				})
			...
			if can_dig then
				for ay = startpos.y, digpos.y+1, -1 do
					...
					local checknode = mcl_technic.get_or_load_node(checkpos) or minetest.get_node(checkpos)
...
minetest.register_node("mcl_technic:hv_quarry", {
	description = S("%s Quarry"):format("HV"),
	tiles = {
		"mcl_technic_hv_quarry_bottom.png"..tube_entry,
		"mcl_technic_hv_quarry_bottom.png"..cable_entry,
		"mcl_technic_hv_quarry_bottom.png"..cable_entry,
		"mcl_technic_hv_quarry_bottom.png"..cable_entry,
		"mcl_technic_hv_quarry_front.png",
		"mcl_technic_hv_quarry_bottom.png"..cable_entry
	},
	paramtype2 = "facedir",
	groups = {pickaxey=1,axey=1, handy=1, swordy=1, tubedevice=1, mcl_technic_machine=1, mcl_technic_hv=1},
...
mcl_technic.register_machine("HV", "mcl_technic:hv_quarry", mcl_technic.receiver)
