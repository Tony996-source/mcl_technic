...
mcl_technic.recipes = { cooking = { input_size = 1, output_size = 1 } }
function mcl_technic.register_recipe_type(typename, origdata)
	local data = {}
	...
	data.recipes = {}
	mcl_technic.recipes[typename] = data
end
...
local function register_recipe(typename, data)
	...
	if not index then
		print("[mcl_technic] ignored registration of garbage recipe!")
		return
	end
	...
	mcl_technic.recipes[typename].recipes[index] = recipe
	if have_ui and mcl_technic.recipes[typename].output_size == 1 then
...
function mcl_technic.register_recipe(typename, data)
	minetest.after(0.01, register_recipe, typename, data) -- Handle aliases
end

function mcl_technic.get_recipe(typename, items)
	...
	local recipe = mcl_technic.recipes[typename].recipes[index]
	if recipe then
		local new_input = {}
		for i, stack in ipairs(items) do
