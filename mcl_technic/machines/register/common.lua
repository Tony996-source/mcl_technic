...
function mcl_technic.handle_machine_upgrades(meta)
	...
	if upg_item1 == "mcl_technic:control_logic_unit" then
		tube_upgrade = tube_upgrade + 1
	elseif upg_item1 == "mcl_technic:battery" then
		EU_upgrade = EU_upgrade + 1
	end

	if upg_item2 == "mcl_technic:control_logic_unit" then
		tube_upgrade = tube_upgrade + 1
	elseif  upg_item2 == "mcl_technic:battery" then
		EU_upgrade = EU_upgrade + 1
	end
...
local function on_machine_upgrade(meta, stack)
	...
	if stack_name == "default:chest" then
		...
		return 1
	elseif stack_name ~= "mcl_technic:control_logic_unit"
	   and stack_name ~= "mcl_technic:battery" then
		return 0
	end
	return 1
end
...
function mcl_technic.send_items(pos, x_velocity, z_velocity, output_name)
	...
	for _, stack in ipairs(inv:get_list(output_name)) do
		...
		if stack then
			...
			if item0 then 
				item0["count"] = "1"
				mcl_technic.tube_inject_item(pos, pos, vector.new(x_velocity, 0, z_velocity), item0)
				stack:take_item(1)
...
function mcl_technic.smelt_item(meta, result, speed)
	local inv = meta:get_inventory()
...
function mcl_technic.handle_machine_mcl_pipeworks(pos, tube_upgrade, send_function)
	if send_function == nil then
		send_function = mcl_technic.send_items
	end
...
function mcl_technic.machine_can_dig(pos, player)
	local meta = minetest.get_meta(pos)
	local inv = meta:get_inventory()
...
function mcl_technic.machine_after_dig_node(pos, oldnode, oldmetadata, player)
	...
	if minetest.registered_nodes[oldnode.name].tube then
		mcl_pipeworks.after_dig(pos, oldnode, oldmetadata, player)
	end
end
...
function mcl_technic.machine_inventory_put(pos, listname, index, stack, player)
	return inv_change(pos, player, stack:get_count(), nil, listname, stack)
end

function mcl_technic.machine_inventory_take(pos, listname, index, stack, player)
	return inv_change(pos, player, stack:get_count(), listname, nil, stack)
end

function mcl_technic.machine_inventory_move(pos, from_list, from_index,
