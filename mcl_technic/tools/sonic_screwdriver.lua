...
mcl_technic.register_power_tool("mcl_technic:sonic_screwdriver", sonic_screwdriver_max_charge)
...
local function screwdriver_handler(itemstack, user, pointed_thing, mode)
	...
	minetest.sound_play("mcl_technic_sonic_screwdriver", {pos = pos, gain = 0.3, max_hear_distance = 10})
	...
	if not mcl_technic.creative_mode then
		...
		mcl_technic.set_RE_wear(itemstack, meta1.charge, sonic_screwdriver_max_charge)
	end

	return itemstack
end

minetest.register_tool("mcl_technic:sonic_screwdriver", {
	description = S("Sonic Screwdriver (left-click rotates face, right-click rotates axis)"),
	inventory_image = "mcl_technic_sonic_screwdriver.png",
...
minetest.register_craft({
	output = "mcl_technic:sonic_screwdriver",
	recipe = {
	{"", "mcl_core:diamond", ""},
	{"mcl_technic:carbon_plate", "mcl_technic:battery", "mcl_technic:carbon_plate"},
	{"mcl_technic:carbon_plate", "mcl_core:iron_ingot", "mcl_technic:carbon_plate"}
	}
})
